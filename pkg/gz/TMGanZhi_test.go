...
func TestNewTMGanZhi2(t *testing.T) {
	y, m, d, h, min := 2022, 5, 21, 9, 21 //立夏: 2022-05-05 20:25:46
	y, m, d, h, min = 2022, 5, 21, 9, 23  //小满: 2022-05-21 09:22:24
	gzo := NewTMGanZhi(y, m, d, h, min)
	fmt.Println(gzo.TMJieQi()) //立夏: 2022-05-05 20:25:46
}
...
*/
func TestNewTMGanZhi(t *testing.T) {
	y2033 := []struct {
	...
	for i := 0; i < len(y2033); i++ {
		...
		min := ts.min
		gzo := NewTMGanZhi(year, ts.m, d, h, min)
...
*/
func TestTMMonthGanZhi(t *testing.T) {

	year, month, day, hour, min := 2022, 5, 21, 9, 21 //
	year, month, day, hour, min = 2022, 5, 21, 9, 22
	...
	ygz, mgz, jqt, jqn, zqt, zqn := TMMonthGanZhi(year, month, day, hour, min)
	fmt.Printf(" %d %d %d %dH %s年 月干支:%s\n", year, month, day, hour, ygz, mgz)
	fmt.Println(jqt, jqn, zqt, zqn)
}
...
*/
func TestTMYearGanZhi(t *testing.T) {
	gzo := NewTMGanZhi(2033, 12, 22, 3, 22)
